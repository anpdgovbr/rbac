...
export function PermissionsEditor({ client, profileIdOrName }: { client: AdminClient; profileIdOrName: string | number }): React.ReactElement {
  ...
  async function onToggle(acao: string, recurso: string, value: boolean) {
    ...
    try {
      const res = await client.togglePermission({ profileIdOrName, acao, recurso, permitido: value })
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('input', {
          ...
          onChange: (e: any) => onToggle(it.acao, it.recurso, !!e.target?.checked),
        })
      )
    )
  )
