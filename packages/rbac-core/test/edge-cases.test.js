...
test("toPermissionsMap handles nil or empty lists", () => {
    assert.deepStrictEqual(toPermissionsMap(null), {}, "Should return empty map for null input");
    assert.deepStrictEqual(toPermissionsMap(undefined), {}, "Should return empty map for undefined input");
    assert.deepStrictEqual(toPermissionsMap([]), {}, "Should return empty map for empty array");
});
...
test("pode and hasAny handle empty permissions map", () => {
    const emptyPerms = {};
    assert.strictEqual(pode(emptyPerms, "Exibir", "Recurso"), false, "pode should return false for empty map");
    assert.strictEqual(hasAny(emptyPerms, [["Exibir", "Recurso"]]), false, "hasAny should return false for empty map");
});
