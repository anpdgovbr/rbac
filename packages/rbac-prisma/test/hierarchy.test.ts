...
test("getPermissoesPorPerfil with complex hierarchy", async () => {
  const prismaMock: PrismaLike = {
    perfil: {
      findUnique: async ({ where }: { where: { nome: string } }) => {
        const perfis: Record<string, { id: number; nome: string; active: boolean }> = {
    ...
    perfilHeranca: {
      findMany: async ({ where }: { where: { childId: { in: number[] } } }) => {
        const childId = where.childId.in[0]
        if (childId === 3) {
          // Analista herda de Supervisor
          return [
            {
              parentId: 2,
    ...
    permissao: {
      findMany: async ({ where }: { where: { perfil: { nome: { in: string[] } } } }) => {
        ...
        const todas = [
          ...
          },
          {
            ...
            permitido: false,
            perfil: { nome: "Supervisor" },
          }, // Nega o que o Admin permite
          // Analista (nível mais baixo)
          {
            acao: "Exibir",
            recurso: "Usuarios",
            permitido: true,
            perfil: { nome: "Analista" },
          },
        ]
  ...
  assert.strictEqual(
    mapa.get("Exibir:Usuarios"),
    true,
    "Analista deve poder Exibir Usuários"
  )
  assert.strictEqual(
    mapa.get("Criar:Usuarios"),
    true,
    "Deve herdar Criar:Usuarios de Supervisor"
  )
  assert.strictEqual(mapa.get("Exibir:Logs"), true, "Deve herdar Exibir:Logs de Admin")
  assert.strictEqual(
    mapa.get("Excluir:Usuarios"),
    true,
    "Grant verdadeiro: Admin (true) deve sobrescrever Supervisor (false)"
  )
})
