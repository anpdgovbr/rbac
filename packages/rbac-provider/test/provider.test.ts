...
test('withTTLCache caches and invalidates', async () => {
  ...
  const base: PermissionsProvider = {
    async getPermissionsByIdentity() { calls++; return { Exibir: { Permissoes: true } } },
    invalidate() {}
  }
  const provider = withTTLCache(base, 1000)
  const a = await provider.getPermissionsByIdentity('a@x')
  const b = await provider.getPermissionsByIdentity('a@x')
